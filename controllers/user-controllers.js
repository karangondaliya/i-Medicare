...
const loginController = async (req, res) => {
    try {
        ...
        if (!user.is_active) {
            return res.status(200).send({ message: `Account is inactive`, success: false });
        }
...
const registerOrganDonationController = async (req, res) => {
  ...
  const getUserMedicalHistory = async (req, res) => {
    try {
        ...
        // Success response with records
        return res.status(200).json({ success: true, records });
    } catch (error) {
        console.error('Error fetching medical records:', error); // Log the error for debugging
